...
in `X²`. -/
theorem eq_expand_two_contract_of_comp_neg_X_eq (hb : b.comp (-X) = b) :
...
theorem exists_eq_comp_X_sq_add_C_of_comp_neg_X_eq (c : R) {b : R[X]} (hb : b.comp (-X) = b) :
    ∃ e : R[X], b = e.comp (X ^ 2 + C c) :=
  ⟨(contract 2 b).comp (X - C c), by
    rw [comp_assoc, show (X - C c : R[X]).comp (X ^ 2 + C c) = X ^ 2 by simp,
      ← expand_eq_comp_X_pow, ← eq_expand_two_contract_of_comp_neg_X_eq hb]⟩
...
theorem _root_.Irreducible.isUnit_or_associated_of_dvd_comp_of_comp_neg_X_eq [NeZero (2 : K)]
  ...
  obtain ⟨e, rfl⟩ := exists_eq_comp_X_sq_add_C_of_comp_neg_X_eq c heven
