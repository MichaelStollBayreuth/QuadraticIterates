...
* `IntermediateField.square_descent`: `d ∈ L` is a square in `L(√r₁, …, √rₘ)` iff `d · ∏_{i ∈ t} rᵢ`
  is a square in `L` for some subset `t`.
* `multiquadratic_degree`, `multiquadratic_degree_family`: the degree formula
  `[L(√r₁, …, √rₘ) : L] = 2 ^ (m - dim V)`.
...
* `rootRelations_all_ones`: for a transitive action of a `2`-group compatible with the radicands,
  a nonzero relation space contains the all-ones vector.
* `IntermediateField.isGalois_adjoin_of_sq_eq_algebraMap`, `algEquiv_adjoin_sq_eq_one`: a field
  generated by square roots is Galois with a Galois group of exponent `2`; inside a finite Galois
  extension `E/L` its degree is at most `#Hom(Gal(E/L), C₂)` (`finrank_adjoin_le_card_monoidHom`),
  and when that bound is attained it contains every square root of an element of `L`
  (`mem_adjoin_of_sq_eq_algebraMap_of_card_le`).
...
theorem isGalois_adjoin_of_sq_eq_algebraMap [IsGalois L E] {S : Set E}
  ...
  have := Algebra.isSeparable_tower_bot_of_isSeparable L (adjoin L S) E
  exact IsGalois.mk
...
theorem finrank_adjoin_le_card_monoidHom [IsGalois L E] [FiniteDimensional L E] {S : Set E}
  ...
  exact Nat.card_le_card_of_injective (fun φ ↦ φ.comp (AlgEquiv.restrictNormalHom (adjoin L S)))
    fun _ _ h ↦ (MonoidHom.cancel_right (AlgEquiv.restrictNormalHom_surjective E)).mp h
...
theorem rootRelations_finrank_le {ι : Type*} [Fintype ι] (r : ι → L) :
...
theorem multiquadraticRelations_finrank_eq_rootRelations (s : Finset ι) (r : ι → L) :
...
theorem multiquadraticRelations_finrank_le (s : Finset ι) (r : ι → L) :
...
theorem multiquadraticRelations_insert_finrank [DecidableEq ι] {s : Finset ι} {j : ι} (hjs : j ∉ s)
...
theorem multiquadratic_degree {s : Finset ι} {x : ι → E} {r : ι → L}
...
relations. -/
theorem multiquadratic_degree_family [Fintype ι] {x : ι → E} {r : ι → L}
