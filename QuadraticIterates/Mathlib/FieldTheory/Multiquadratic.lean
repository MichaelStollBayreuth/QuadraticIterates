...
/-- A field generated by square roots of elements of `L` has exponent `2`: every
`L`-automorphism of it is an involution. -/
theorem algEquiv_adjoin_sq_eq_one {t : Finset E} (ht : ∀ y ∈ t, ∃ q : L, y ^ 2 = algebraMap L E q)
    (τ : adjoin L (t : Set E) ≃ₐ[L] adjoin L (t : Set E)) : τ ^ 2 = 1 := by
  refine AlgEquiv.coe_toAlgHom_injective (adjoin_algHom_ext L fun x hx ↦ ?_)
  obtain ⟨q, hq⟩ := ht x hx
  have hgq : (⟨x, subset_adjoin L _ hx⟩ : adjoin L (t : Set E)) ^ 2 = algebraMap L _ q :=
    Subtype.ext (by simpa using hq)
  rcases τ.apply_eq_or_eq_neg_of_sq_eq_algebraMap hgq with h | h <;> simp [sq, h]

/-- A subfield of a normal extension `E/L` generated by square roots of elements of `L` is
Galois over `L`. -/
theorem isGalois_adjoin_of_sq_eq_algebraMap [Normal L E] [PerfectField L] {t : Finset E}
    (ht : ∀ y ∈ t, ∃ q : L, y ^ 2 = algebraMap L E q) : IsGalois L (adjoin L (t : Set E)) := by
  have : Normal L (adjoin L (t : Set E)) := normal_iff_forall_map_le'.mpr fun σ ↦ by
    rw [adjoin_map, adjoin_le_iff]
    rintro x ⟨y, hyt, rfl⟩
    obtain ⟨q, hq⟩ := ht y hyt
    have hy : y ∈ adjoin L (t : Set E) := subset_adjoin L _ hyt
    rcases σ.apply_eq_or_eq_neg_of_sq_eq_algebraMap hq with h | h <;> simp [h, hy]
  have := Algebra.IsAlgebraic.isSeparable_of_perfectField (K := L) (L := adjoin L (t : Set E))
  exact IsGalois.mk
