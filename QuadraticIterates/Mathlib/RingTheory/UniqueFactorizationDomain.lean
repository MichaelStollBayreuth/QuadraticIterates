...
An automorphism that fixes an element up to associates permutes its normalized factors, and an
involutive one pairs them up when the element has no nontrivial invariant divisor
(`exists_normalizedFactors_eq_add_map`). Multiplicities as a `Finsupp` (`factorization`): behaviour
under products and gcds, and the shape forced by a periodic congruence.
...
theorem exists_normalizedFactors_eq_add_map {α : Type*} [CommMonoidWithZero α]
    [NormalizationMonoid α] [UniqueFactorizationMonoid α] (σ : α ≃* α)
    (hσ : Function.Involutive σ) {p : α} (hp0 : p ≠ 0) (hσp : Associated (σ p) p)
    (hp : ¬Irreducible p) (h : ∀ d : α, d ∣ p → Associated (σ d) d → IsUnit d ∨ Associated d p) :
  ...
  · have hirr := irreducible_of_normalized_factor q hq
    ...
    exact (h q (dvd_of_mem_normalizedFactors hq) hσq).elim hirr.not_isUnit
      fun hap ↦ hp (hap.irreducible hirr)
