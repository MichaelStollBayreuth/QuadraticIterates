...
* `QuadraticIterates.nonempty_mulEquiv_of_neg_lt_of_twoAdicClass_of_squarefreeCert`
  (**Theorem R⁺**, the positive world `a > -1`, `a ≠ 0`);
* `QuadraticIterates.nonempty_mulEquiv_of_two_mul_le_neg_of_twoAdicClass_of_squarefreeCert`
...
(`QuadraticIterates.SquarefreeCert`) holds; the conclusion is `Ω_n ≅ [C₂]ⁿ` for all `n`. Both are
instances of the uniform theorem
`QuadraticIterates.nonempty_mulEquiv_of_twoAdicClass_of_squarefreeCert` for `a = εr/s` with a
positive sequence `w = QuadraticIterates.wSeq r s ε`: in the 2-adic classes `β_n` is not a square
for every non-squarefree `n` (`QuadraticIterates.not_isSquare_betaInt_of_two_le`), under a
certificate for every squarefree `n ≥ 2`
(`QuadraticIterates.not_isSquare_betaInt_of_squarefree_of_squarefreeCert`), and the shared-part
lemma `QuadraticIterates.nonempty_mulEquiv_of_forall_not_isSquare_abs_betaInt` concludes.
...
(`QuadraticIterates.not_isSquare_betaInt_of_squarefree_of_squarefreeCert`): one of the four
mechanisms of the squarefree levels, each with its modulus. -/
inductive SquarefreeCert (r s ε : ℤ) : Prop
...
theorem not_isSquare_betaInt_of_squarefree_of_squarefreeCert (hs : s ≠ 0) (hrs : IsCoprime r s)
    (hε : ε ^ 2 = 1) (hw : ∀ n ≥ 1, wSeq r s ε n ≠ 0) (hcert : SquarefreeCert r s ε) {n : ℕ}
...
that is not squarefree: `QuadraticIterates.not_isSquare_betaInt_of_two_le` with
`k = n / rad n ≥ 2`. -/
...
`n ≥ 2`. -/
theorem not_isSquare_betaInt_of_twoAdicClass_of_squarefreeCert (hs : 0 < s) (hrs : IsCoprime r s)
    (hε : ε ^ 2 = 1) (hw : ∀ n ≥ 1, 0 < wSeq r s ε n) (hc : TwoAdicClass r s ε)
    (hcert : SquarefreeCert r s ε) {n : ℕ} (hn : 2 ≤ n) : ¬IsSquare (betaInt r s ε n) :=
  (em (Squarefree n)).elim
    (not_isSquare_betaInt_of_squarefree_of_squarefreeCert hs.ne' hrs hε
...
theorem nonempty_mulEquiv_of_twoAdicClass_of_squarefreeCert (hs : 0 < s) (hrs : IsCoprime r s)
    (hε : ε ^ 2 = 1) (hw : ∀ n ≥ 1, 0 < wSeq r s ε n) (hc : TwoAdicClass r s ε)
    (ha : ¬IsSquare (-(ε * r * s))) (hcert : SquarefreeCert r s ε) (n : ℕ) :
  ...
  nonempty_mulEquiv_of_forall_not_isSquare_abs_betaInt hs.ne' hrs hε (fun n hn ↦ (hw n hn).ne') ha
    (fun _ hn ↦ (abs_of_pos (betaInt_pos hs.ne' hrs hε hw (one_le_two.trans hn))).symm ▸
      not_isSquare_betaInt_of_twoAdicClass_of_squarefreeCert hs hrs hε hw hc hcert hn) n
...
and a squarefree-level certificate (`QuadraticIterates.SquarefreeCert`). Then `Ω_n ≅ [C₂]ⁿ` for
all `n`. The sequence `w` of `a` is positive for `a > 0` and for `-1 < a < 0`. -/
theorem nonempty_mulEquiv_of_neg_lt_of_twoAdicClass_of_squarefreeCert (hs : 0 < s)
    (hrs : IsCoprime r s) (hr : -s < r) (hc : TwoAdicClass r s 1) (ha : ¬IsSquare (-(r * s)))
    (hcert : SquarefreeCert r s 1) (n : ℕ) :
  ...
  exact (show ((1 : ℤ) : ℚ) * r / s = (r / s : ℚ) by simp) ▸
    nonempty_mulEquiv_of_twoAdicClass_of_squarefreeCert hs hrs (one_pow 2) hw hc
...
squarefree-level certificate (`QuadraticIterates.SquarefreeCert`) for `(-r, s, -1)`. Then
`Ω_n ≅ [C₂]ⁿ` for all `n`. The sequence `w` of `(-r, s, -1)` is the sequence of `a` in Stoll's
normalization `a = -|r|/s`, positive for `a ≤ -2`. -/
theorem nonempty_mulEquiv_of_two_mul_le_neg_of_twoAdicClass_of_squarefreeCert (hs : 0 < s)
    (hrs : IsCoprime r s) (hr : 2 * s ≤ -r) (hc : TwoAdicClass (-r) s (-1))
    (ha : ¬IsSquare (-(r * s))) (hcert : SquarefreeCert (-r) s (-1)) (n : ℕ) :
    Nonempty (GaloisGroup (r / s : ℚ) n ≃* WreathPower n) :=
  (show ((-1 : ℤ) : ℚ) * ((-r : ℤ) : ℚ) / s = (r / s : ℚ) by simp) ▸
    nonempty_mulEquiv_of_twoAdicClass_of_squarefreeCert hs hrs.neg_left neg_one_sq
