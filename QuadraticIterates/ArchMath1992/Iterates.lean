...
lemma map_iteratedPoly_succ_comp (k : ℕ) : fℚ[a, k + 1] = fℚ[a, k].comp (X ^ 2 + C (a : ℚ)) := by
  simp only [map_iteratedPoly]; exact iteratedPoly_succ_comp _ k
...
lemma mem_rootSet_iteratedPoly_succ (n : ℕ) (β : AlgebraicClosure ℚ) :
  ...
  rw [mem_rootSet_of_ne (monic_map_iteratedPoly a _).ne_zero,
    mem_rootSet_of_ne (monic_map_iteratedPoly a _).ne_zero, map_iteratedPoly_succ_comp, aeval_comp]
  simp
