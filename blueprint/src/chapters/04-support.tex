...
\begin{lemma}[Degree of a multiquadratic extension]
  ...
  \lean{multiquadraticRelations, mem_multiquadraticRelations, finrank_adjoin_image_eq_two_pow_sub,
    finrank_adjoin_range_eq_two_pow_sub, IntermediateField.adjoin_insert,
    ...
    IntermediateField.finrank_adjoin_sqrt_eq, finrank_multiquadraticRelations_le,
...
\begin{lemma}[Square descent]
  \label{lem:descent}
  \uses{def:twoindep}
  \lean{IntermediateField.exists_mem_adjoin_sq_eq_algebraMap_iff,
    IntermediateField.exists_mem_adjoin_sq_eq_algebraMap_of_isSquare_mul_prod,
    IntermediateField.exists_mem_adjoin_simple_sq_eq_algebraMap_iff, IntermediateField.isSquare_algebraMap_iff,
    ...
    sqClass_mul, sqClass_prod, sqClass_prod_zpow, sqClass_eq_zero_iff, sqClass_pow,
    sqClass_mul_sq, sqClass_div_sq, ZModModule.zsmul_eq_natAbs_nsmul,
...
\begin{lemma}[Invariants of a $p$-group]
  \label{lem:invariant}
  \uses{lem:relations}
  \lean{IsPGroup.exists_ne_zero_mem_fixedPoints_of_smul_mem, IsPGroup.one_mem_of_comp_smul_mem_of_ne_bot,
    comp_smul_mem_rootRelations, one_mem_rootRelations_of_ne_bot}
  \leanok
...
\begin{lemma}[Integrality of the Möbius quotients]
  ...
  \lean{moebiusFactorK, moebiusFactorK_eq_div, moebiusFactorK_eq_prod, moebiusFactorR, numProd,
    ...
    moebiusFactorR_mul_denProd_of_dvd, algebraMap_moebiusFactorR_of_dvd,
    prod_moebiusFactorR_of_dvd, moebiusFactorR_ne_zero_of_dvd, map_moebiusFactorR, map_numProd,
    map_denProd, factorization_gcd_min, UniqueFactorizationMonoid.normalizedFactors_gcd,
    isInteger_div_iff_dvd}
  \leanok
  Let $R$ be a domain and $(c_n)$ a sequence in $R \setminus \{0\}$ (on positive indices). Write
  the Möbius quotient $b_n := \prod_{d \dvd n} c_d^{\mu(n/d)}$ as $N/D$ with $N$ the product of the
  $c_d$ with $\mu(n/d) = 1$ and $D$ the product of those with $\mu(n/d) = -1$. If $D \dvd N$, then
  $b_n$ lies in $R$ and is nonzero, and a ring homomorphism into a domain that kills no $c_d$
  commutes with $b_n$; if $D \dvd N$ at every divisor of $n$, Möbius inversion holds:
  $c_n = \prod_{d \dvd n} b_d$. If $R$ is a UFD and $(c_n)$ is a strong divisibility sequence,
  then $D \dvd N$ for every $n$.
\end{lemma}
\begin{proof}
  \uses{lem:moebiussign}
  \leanok
  Given $D \dvd N$, the $R$-valued quotient is the solution of $b_n D = N$, which the
  homomorphism preserves. For the last sentence: at each prime $p$, strong divisibility bounds
...
\begin{lemma}[Möbius sums]
  ...
  \lean{Squarefree.card_filter_moebius_div_eq_one_eq_card_filter_eq_neg_one,
    ...
    Squarefree.moebius_div_eq_one_or_eq_neg_one,
    Squarefree.filter_moebius_div_eq_one_union_filter_eq_neg_one_of_subset,
    Squarefree.even_card_of_sum_moebius_div_eq_zero,
    Squarefree.card_eq_two_mul_card_filter_moebius_div_eq_one, Squarefree.card_divisors_eq_two_mul,
    Squarefree.prod_filter_moebius_div_eq_one_mul_prod_filter_eq_neg_one,
    Squarefree.sum_filter_moebius_div_eq_one_add_sum_filter_eq_neg_one,
    Squarefree.even_sum_two_pow_mul_sub_one, Squarefree.odd_sum_two_pow_sub_one,
    ...
    ArithmeticFunction.sum_erase_divisorsAntidiagonal_moebius_eq_neg,
    ArithmeticFunction.prod_pow_moebius_eq_prod_divisors_radical, ArithmeticFunction.sum_divisorsAntidiagonal_filter_dvd_moebius, ArithmeticFunction.sum_mul_moebius_boole_nonneg, ArithmeticFunction.sum_mul_moebius_nonneg,
    ArithmeticFunction.sum_divisors_moebius, moebiusFactorK_mul_const, abs_moebiusFactorK,
  ...
  factors; and
  $\sum_{d \dvd n} \mu(n/d) = 0$ for $n \ge 2$, so that scaling the sequence $(x_d)$ by a
  nonzero constant does not change its Möbius products of index $n \ge 2$ (and forming Möbius
  products commutes with taking absolute values).
\end{lemma}
\begin{proof}
  \leanok
...
\begin{lemma}[Non-squares mod $m$]
  ...
  \lean{ZMod.isSquare_neg_one_of_isSquare_div, ZMod.isSquare_mul_of_isSquare_div,
    Int.isSquare_mul_of_isSquare_div, IsUnit.isSquare_mul_sq_iff, IsUnit.isSquare_mul_pow_iff_of_even,
    IsUnit.isSquare_mul_pow_iff_of_odd, IsUnit.isSquare_pow_iff_of_odd, ZMod.not_isSquare_neg_one_of_dvd,
...
\begin{lemma}[Even polynomials are polynomials in $X^2$]
  \label{lem:evenpoly}
  \lean{Polynomial.exists_eq_comp_X_sq_add_C_of_comp_neg_X_eq, Polynomial.eq_expand_two_contract_of_comp_neg_X_eq,
