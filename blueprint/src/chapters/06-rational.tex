...
\begin{proof}
  ...
  \formalnote{The Galois side (\lname{Iterates.lean}, \lname{Irreducibility.lean},
  \lname{DegreeCriterion.lean}, \lname{Main.lean} up to \lname{section1\_b\_iff\_c}) is to be
  parametrized by $a \in \QQ$: \lname{splittingField}, \lname{GaloisGroup}, \lname{rootShift}
  take a rational $a$; \lname{cSeq} and \lname{bSeq} become polymorphic in a commutative
  (semi)ring, $c$ being the $\gamma$-sequence of $X^2 + a$ with $\varepsilon = -1$ over that ring,
  with the cast lemma \lname{intCast\_cSeq} (the integer $c_n$ cast to $\QQ$ is the rational
  $c_n$). The hypothesis \lname{ha} ($-a$ is not a square in $\QQ$) of the seven declarations of
  \lname{DegreeCriterion.lean} and of \lname{section1\_a\_iff\_b} becomes \lname{hirr} ($f_k$ is
  irreducible for all $k \le n$, or for all $k$); \lname{cSeq\_ne\_zero} at the level of
  \lname{rootShift\_ne\_zero} becomes \lname{cSeq\_succ\_ne\_zero\_of\_irreducible}. Use sites
  fall into three categories: cast plumbing (\lname{Int.cast} $\to$ \lname{algebraMap},
  \lname{intCast\_mem} $\to$ \lname{algebraMap\_mem}, \lname{map\_intCast} $\to$
  \lname{AlgEquiv.commutes}; about twelve sites, mechanical), the hypothesis swap (nine
  declarations), and Möbius inversion in \lname{section1\_b\_iff\_c}, which only needs
  $c_m = \prod_{d \mid m} b_d$ and $b_m = \prod c_d^{\mu(m/d)}$ in $\QQ$ (\lname{moebiusFactorK}),
  not integrality. The integer statements \lname{section1\_a\_iff\_b}, \lname{section1\_tfae},
  \lname{section1\_of\_not\_isSquare\_abs\_bSeq} keep their names and become corollaries via
  Corollary~\ref{cor:1.3}; the rational versions are \lname{section1\_a\_iff\_b\_of\_irreducible},
  \lname{section1\_tfae\_of\_irreducible}. The growth argument of \lname{Irreducibility.lean}
  stays integral. \emph{Comparator:} \lname{Challenge.lean} reproduces \lname{GaloisGroup} with an
  integer argument; with a rational argument its copy and the challenge statements change by one
  cast; the statements stay equivalent and become simpler, but this is a change of the challenge
  file and needs the author's approval.}
\end{proof}
...
\begin{definition}[the $w$-sequence]
  \label{def:wseq}
  \uses{def:cseq}
  For coprime $r, s \in \ZZ$, $s \ge 1$, let $w_1 = 1$ and $w_{n+1} = r w_n^2 + s^{2^n - 1}$ (junk
  value $w_0 = 0$). With $\gamma_n = f^n(0)/a$, i.e.\ $\gamma_1 = 1$, $\gamma_{n+1} = a\gamma_n^2 + 1$,
  one has $\gamma_n = w_n / s^{2^{n-1} - 1}$ and $c_n = a \gamma_n = r w_n / s^{2^{n-1}}$ for
  $n \ge 1$.
  \formalnote{\lname{wSeq}, a function of $r$, $s$ and $n$ with values in $\ZZ$, by recursion
  from $w_0 = 0$, so that $w_{n+1} = r w_n^2 + s^{2^n - 1}$ holds for all $n$
  (\lname{wSeq\_succ}, by \lname{rfl}); the $\gamma$-sequence is \lname{gammaSeq} of $aX^2 + 1$
  with $\varepsilon = 1$, and $c_n = a\gamma_n$ for $n \ge 1$ over $\QQ$
  (\lname{cSeq\_eq\_mul\_gammaSeq}). The identity $w_n = s^{2^{n-1}-1}\gamma_n$ is proved once,
  for every commutative ring $S$ with a ring homomorphism $\varphi : \ZZ \to S$ and an element
  $u$ with $\varphi(s)\,u = 1$ (\lname{map\_wSeq\_eq\_mul\_gammaSeq}: $\varphi(w_n) =
  \varphi(s)^{2^{n-1}-1}\gamma_n$ for the $\gamma$-sequence of $\varphi(r)u\,X^2 + 1$ over $S$,
  an induction of a dozen lines), and it is used three times: over $\ZZ[1/s]$
  (Lemma~\ref{lem:gamma-away}), over $\QQ$ (Lemma~\ref{lem:rat-sqclass}) and over $\ZZ/M\ZZ$
  (Lemma~\ref{lem:w-gamma-zmod}). Because of the varying term $s^{2^n-1}$, $w$ is not itself a
  $\gamma$-sequence over $\ZZ$; it is one over $\ZZ[1/s]$ up to units, which is how the general
  theory of Chapter~2 enters.}
\end{definition}
...
\end{lemma}
\begin{proof}
  $w_{n+1} \equiv r w_n^2 \pmod s$ and $w_{n+1} \equiv s^{2^n - 1} \pmod r$; induction from
  $w_1 = 1$ and $\gcd(r, s) = 1$.
  \formalnote{\lname{isCoprime\_wSeq\_left} ($w_n$ coprime to $r$) and
  \lname{isCoprime\_wSeq\_right} ($w_n$ coprime to $s$) for $n \ge 1$, by
  \lname{Nat.le\_induction} and \lname{IsCoprime.mul\_add\_left\_left}-style lemmas.}
\end{proof}
...
\begin{proof}
  ...
  \lname{wSeqAway} (the sequence $n \mapsto \varphi(w_n)$ in $R$),
  \lname{wSeqAway\_associated\_gcd}, \lname{factorization\_wSeqAway\_shape},
  \lname{isRelPrime\_moebiusFactorR\_wSeqAway}, each a few lines, exactly as
  \lname{factorization\_cSeq\_shape} and \lname{isCoprime\_bSeq} in Chapter~1. A prototype of
  this route --- the instances, the identity for $w$ over $R$, the API applications and the
  descent of Lemma~\ref{lem:betaint} --- compiles against the repository (240 lines, 2.5 s;
  \lname{explore/stoll-extensions-2026-09-07/UfdAway.lean} outside the repository).}
\end{proof}
...
\begin{definition}[the integer Möbius factors]
  ...
  for $n \ge 2$.
  \formalnote{\lname{betaInt r s n := moebiusFactorR (wSeq r s) n}, the same device as
  ...
  through $D \mid N$ (Lemma~\ref{lem:moebiusfactor}); these four lemmas get primary forms
  \lname{\_of\_dvd} with the hypothesis $D \mid N$, of which the present forms become corollaries,
  and a lemma \lname{map\_moebiusFactorR}: for $D \mid N$ and a ring homomorphism $\varphi$
  into a domain with $\varphi(D) \ne 0$, $\varphi(\beta_n)$ is the Möbius factor of
  $\varphi \circ w$, both being the solution of $\beta D = N$. Then \lname{twistExp}, the integer
  $E(n)$ as a sum over \lname{n.divisorsAntidiagonal}, with \lname{twistExp\_emod\_two}
...
\begin{proof}
  ...
  \lname{isCoprime\_of\_isRelPrime\_localizationAway} (integers prime to $x$ whose images are
  relatively prime are coprime, by \lname{Nat.coprime\_of\_dvd}). Then
  \lname{denProd\_wSeq\_dvd\_numProd\_wSeq}, and the API of \lname{betaInt}
  (\lname{intCast\_betaInt}, \lname{betaInt\_ne\_zero}, \lname{wSeq\_eq\_prod\_betaInt}) from the
  \lname{\_of\_dvd} forms; \lname{algebraMap\_betaInt} from \lname{map\_moebiusFactorR};
  \lname{isCoprime\_betaInt} from the transfer lemma and
  \lname{isRelPrime\_moebiusFactorR\_wSeqAway}; \lname{isCoprime\_betaInt\_mul :
  IsCoprime (betaInt r s n) (r * s)} from $\beta_n D = N$.}
\end{proof}
...
\begin{proof}
  ...
  $i \mapsto (-s)^{\mu(i+1)} \beta_{i+1}$ is not pairwise coprime, so the lemma to prove is: for
  pairwise coprime $\beta_i$ coprime to $u$, if $u^e \prod_{i \in S} \beta_i$ is a square then each
  $|\beta_i|$, $i \in S$, is a square
  (\lname{Int.isSquare\_abs\_of\_isSquare\_mul\_prod\_of\_pairwise\_isCoprime}, for
  \lname{QuadraticIterates/Mathlib/Algebra/Squares.lean}).}
\end{proof}
...
\begin{proof}
  ...
  \formalnote{The instance of \lname{map\_wSeq\_eq\_mul\_gammaSeq} (Definition~\ref{def:wseq})
...
\begin{lemma}[reflection with the twist]
  ...
  \uses{lem:w-gamma-zmod, def:betaint, lem:li-crit}
...
\end{lemma}
\begin{proof}
  \uses{lem:li-succ, lem:w-gamma-zmod}
  ...
  \formalnote{\lname{not\_isSquare\_betaInt\_of\_dvd\_add\_succ}: hypotheses $n = k \cdot \rad(n)$,
  $M \mid w_k s^{2^{k-1}} + w_{k+1}$, $M$ coprime to $s$, $\beta_n > 0$, and $(-s)^{E(n)}$ not a
  square in \lname{ZMod M}; the two consumed forms have $-1$ not a square with $n$ not squarefree,
  and $-s$ not a square with $n$ squarefree. Coprimality of $M$ with $w_{n_0}$ is automatic for
  this modulus (\lname{isCoprime\_gammaSeq\_add\_succ}). The positive-world version takes $w$,
  the $a \le -2$ version the sequence $w^S$ with $w^S_n = -w_n$ ($n \ge 2$), i.e.\ $\gamma^S$ of
  $|a| X^2 - 1$; both are instances of the same general lemma with $g = \pm\bar a X^2 \pm 1$.}
\end{proof}
...
\end{proposition}
\begin{proof}
  \uses{lem:li-period, lem:w-gamma-zmod}
  ...
  Proposition~5.1).
  \formalnote{Four lemmas in a new \lname{Rational/Residues.lean}, in the style of
  \lname{Li/Residues.lean}: the $\gamma$-sequence of $\bar a X^2 + 1$ over \lname{ZMod 8} with
  $\bar a$ even, resp.\ odd, and the $w$-recursion modulo $8$ for even $s$; conclusions of the
  form $N \equiv 6 \bmod 8$ or $N \equiv 3 \bmod 4$, consumed by
  \lname{ZMod.not\_isSquare\_neg\_one\_of\_emod\_eight\_eq\_six} and
  \lname{ZMod.not\_isSquare\_neg\_one\_of\_emod\_four\_eq\_three}. The class conditions are best
  bundled as a \lname{Prop}-structure \lname{RatClass} in $r$ and $s$ with the disjunction as
  its single field, so that no signature grows.}
\end{proof}
...
\begin{proof}
  ...
  \formalnote{\lname{intCast\_wSeq\_zmod\_of\_dvd}: $w_n \equiv r^{2^{n-1}-1}$ in \lname{ZMod p}
  for $p \mid s$, then \lname{not\_isSquare\_betaInt\_of\_dvd\_of\_not\_isSquare}; the Legendre
  condition is ``$r$ is not a square in \lname{ZMod p}''.}
\end{proof}
...
\end{lemma}
\begin{proof}
  $s^{2^n-1} \equiv 0 \pmod 8$ for $n \ge 2$ and $w_n$ is odd. The exponent of $r$ in $\beta_n$ is
  $\sum_{d \mid n, d \ge 3} \mu(n/d) = -\mu(n) - \mu(n/2)[2 \mid n]$.
  \formalnote{\lname{intCast\_wSeq\_zmod\_eight\_of\_even}: $w_n \equiv r$ in \lname{ZMod 8} for
  $n \ge 3$ and even $s$; the Möbius exponent computation is
  \lname{ArithmeticFunction.sum\_divisors\_filter\_dvd\_moebius\_div} of Chapter~4 in the form
  ``sum over $d \ge 3$''. An odd integer $\not\equiv 1 \pmod 8$ is not a square:
  \lname{Int.not\_isSquare\_of\_emod\_eight\_ne\_one}, for \lname{Squares.lean}.}
\end{proof}
...
\begin{theorem}[Theorem R${}^+$]
  ...
  (SQ${}^+$4) $s$ even and $r + s \not\equiv 1 \pmod 8$ holds. Then $\Om_n \cong \Ct{n}$ for all
  $n \ge 1$.
\end{theorem}
\begin{proof}
  ...
  Lemmas \ref{lem:rat-ps}, \ref{lem:rat-refl1}, \ref{lem:rat-mod8}; then Lemma~\ref{lem:rat-shared}.
  \formalnote{\lname{nonempty\_mulEquiv\_of\_ratClass\_of\_squarefreeCert} with the two
  \lname{Prop}-structures \lname{RatClass} and \lname{SquarefreeCert} in $r$ and $s$ (the four
...
\begin{proof}
  ...
  \formalnote{Second instance of the same lemmas with $w^S$; the two worlds should share every
  statement through the general \lname{gammaSeq} lemmas over \lname{ZMod M}, specialized only at
  the end. Clause (SQ${}^-$4) is the last item of the order of work, since its lemma is the only one
  whose proof is not yet written out in the notes.}
\end{proof}

\begin{lemma}[the $\gamma_2$ route, rational form of {\cite[Lemmas 3.2, 3.3]{li2020}}]
  \label{lem:rat-g2}
  \uses{lem:li-C-sqfree, lem:w-gamma-zmod}
  Let $a \le -2$ and let $M$ be coprime to $s$ and divide $R - s$, the numerator of $\gamma^S_2 =
  |a| - 1$. If $-s$ is a non-square modulo $M$, then $\beta^S_n$ is a non-square modulo $M$ for
  every squarefree $n \ge 3$.
\end{lemma}
\begin{proof}
  \uses{lem:li-C-sqfree}
  Modulo $M$ the sequence $\gamma^S$ is $1, 0, -1, 0, -1, \dots$, and modulo $M^2$ it is
  $\gamma^S_2$ at even and $-1$ at odd indices $\ge 3$ (Lemma~\ref{lem:li-modsq} over
  $\ZZ/M^2\ZZ$). For odd $n$ this is a constant tail and Lemma~\ref{lem:rat-reflection} applies;
  for even $n = 2m$, $m > 1$, the argument of Lemma~\ref{lem:li-C-sqfree} modulo $\gamma_2$ gives
  $\beta^S_n \equiv -s^{E(n)} u^2$ after cancelling the (trivial) power of $\gamma^S_2$.
  \formalnote{To be worked out in detail before formalization: the integral version is
  \lname{not\_isSquare\_betaSeq\_of\_squarefree\_of\_not\_isSquare\_neg\_one} (Chapter~5), whose
  proof modulo $\gamma_2$ should generalize to a ring where $s$ is a unit, with the twist
  $s^{E(n)}$ inserted exactly as in Lemma~\ref{lem:rat-reflection}.}
\end{proof}

\section{Order of work}

\begin{enumerate}
\item The Galois side for $a \in \QQ$ (Theorem~\ref{thm:rat-section1}): one commit per file in
  dependency order --- \lname{Iterates.lean} (polymorphic \lname{cSeq}, \lname{bSeq}; rational
  ...
  \lname{Main.lean} (Section~1 over $\QQ$, integer corollaries). The comparator challenge follows
  once the author has approved the change of \lname{GaloisGroup}.
...
\item \lname{Rational/Main.lean}: Theorem~\ref{thm:rat-pos}, then Theorem~\ref{thm:rat-neg} with
  clauses 1--3, then Lemma~\ref{lem:rat-g2} and clause 4.
\end{enumerate}
