...
This chapter records the rational version of the Section~3 theorem obtained in the working
notes (\texttt{tex/stoll-method-extensions.tex}: Theorems R${}^+$ and R${}^-$ there) and reduces
it to the material of Chapters~1--5 plus the new lemmas stated here. It was written as a plan
before any Lean, and every node is now formalized (\lname{QuadraticIterates/Rational/}, with the
Galois side of Chapter~1 parametrized by $a \in \QQ$); the formalization remarks name the
declarations.
...
\begin{theorem}[Section 1 for rational $a$]
  ...
  $\QQ^\times/(\QQ^\times)^2$, if and only if $b_1, \dots, b_n$ are $2$-independent (for the
  second equivalence only $c_k \ne 0$ for all $k \ge 1$ is needed). In particular
  (Lemma~\ref{lem:1.2}), if none of $c_1, \dots, c_n$ is a square, then $\Om_n \cong \Ct{n}$ iff
...
\begin{proof}
  ...
  \formalnote{\lname{splittingField}, \lname{GaloisGroup}, \lname{rootShift} take a
...
\begin{definition}[the $w$-sequence]
  ...
  place of $r$ is Stoll's positive normalization for $a \le -2$. The sequence $w$ is positive in
  the three sign worlds: for $r, s > 0$, $\varepsilon = 1$ trivially; for $-s < r < 0$,
  $\varepsilon = 1$ since $0 < w_n \le s^{2^{n-1}-1}$; for $\varepsilon = -1$ and $2s \le r$ since
  $w_n \ge s^{2^{n-1}-1}$.
...
\begin{proof}
  ...
  since $s \ne 0$ (Mathlib's \lname{Localization.Away.isDomain}, registered as the instance
  \lname{Localization.Away.instIsDomain} for \lname{[NeZero x]} in the mirror file). The API of Chapter~2 (\lname{gammaSeq\_associated\_gcd},
...
The shape of $w$ could be proved over $\ZZ$ directly: the twisted rigidity
...
factor in the congruence. The localization route reuses Chapter~2 verbatim instead of
re-deriving it with a twist, and its own new lemmas (a domain instance, the transfer of primes
and of divisibility, the descent below) are of Mathlib generality.

\begin{definition}[the integer Möbius factors]
  \label{def:betaint}
  \uses{def:wseq, lem:moebiusfactor}
  ...
  both being the solution of $\beta D = N$. The twist exponent $E(n)$ has no definition of its
  own: the proofs need the parity of the exponents $F = \sum_{t \mid n} (2^{kt-1} - 1)$ of $s$
  that the denominators produce, \lname{Squarefree.even\_sum\_two\_pow\_mul\_sub\_one}
  ($k \ge 2$) and \lname{Squarefree.odd\_sum\_two\_pow\_sub\_one} ($k = 1$) in
  \lname{Mathlib/NumberTheory/Moebius.lean}, from the balanced sign partition
  \lname{Squarefree.card\_divisors\_eq\_two\_mul}.}
\end{definition}
...
\begin{lemma}[square classes]
  ...
  \uses{def:betaint, def:bseq, def:wseq, def:gammaseq, def:twoindep}
  \lean{QuadraticIterates.cSeq_div_eq, QuadraticIterates.cSeq_div_ne_zero,
    QuadraticIterates.sqClass_cSeq_div, QuadraticIterates.sqClass_neg_div,
    QuadraticIterates.sqClass_betaInt, QuadraticIterates.sqClass_bSeq_div,
    QuadraticIterates.cSeq_eq_mul_gammaSeq}
  \leanok
  In $\QQ^\times/(\QQ^\times)^2$: $c_1 = b_1 \sim -rs$ and $b_n \sim (-s)^{\mu(n)} \beta_n$ for
  $n \ge 2$ (positive world); in general, for $a = \varepsilon r / s$ and the sequence $w$ of
  $(r/s)X^2 + \varepsilon$, $b_1 \sim -\varepsilon r s$ and $b_n \sim (-\varepsilon s)^{\mu(n)}
  \beta_n$ for $n \ge 2$.
\end{lemma}
\begin{proof}
  \leanok
  \uses{def:cseq, lem:w-gamma-zmod, lem:moebiussign, lem:betaint, lem:moebiusfactor, lem:descent,
    thm:rat-section1}
  ...
  additively, for a general sign: \lname{sqClass\_bSeq\_div}, $[b_n] = \mu(n)\,[-\varepsilon s]
  + [\beta_n]$ for $n \ge 2$; the proof is \lname{sqClass\_bSeq\_eq\_sum\_divisorsAntidiagonal}
  plus \lname{sqClass\_cSeq\_div}, $[c_d] = [r w_d]$ for $d \ge 2$, which comes from
  $c_d = r w_d / s^{2^{d-1}}$ (\lname{cSeq\_div\_eq}, the rescaling identity
  \lname{cSeq\_eq\_mul\_gammaSeq} $c_n = \varepsilon a\, \gamma_n$ over any ring together with
  \lname{intCast\_wSeq}), and the balanced sign partition.}
\end{proof}

\begin{lemma}[shared-part lemma]
  \label{lem:rat-shared}
  \uses{lem:rat-sqclass, lem:betaint, thm:rat-section1}
  \lean{QuadraticIterates.nonempty_mulEquiv_of_forall_not_isSquare_abs_betaInt,
    QuadraticIterates.twoIndependent_bSeq_div}
  \leanok
  ...
  (in general, for $a = \varepsilon r/s$: if $-\varepsilon r s$ is not a square).
\end{lemma}
\begin{proof}
  \leanok
  \uses{lem:betaint, lem:rat-sqclass, lem:1.2, lem:coprimesquare, lem:zmodsquare}
  ...
  \lname{Int.isSquare\_abs\_of\_isSquare\_mul\_of\_isCoprime}); here $u = (-\varepsilon s)^k
  r^c$, where $r^c$ accounts for $b_1 \sim (-\varepsilon s) r$ (\lname{twoIndependent\_bSeq\_div}).
  The passage from the $2$-independence of the $b$'s to the irreducibility of all iterates and
  then to $\Om_n \cong \Ct{n}$ is \lname{section1\_b\_iff\_c}, Lemma~\ref{lem:1.2} and
  \lname{section1\_tfae\_of\_irreducible}.}
\end{proof}
...
\begin{lemma}[$w$ and $\gamma$ modulo $M$]
  ...
  $\gamma$-sequence of $\bar a X^2 + \varepsilon$ with sign $\varepsilon$ over $\ZZ/M\ZZ$.
\end{lemma}
\begin{proof}
  \leanok
  ...
  $\gamma$-sequence over \lname{ZMod M} is \lname{gammaSeq} of $\bar a X^2 + \varepsilon$ with
  $\bar a = \bar r \bar s^{-1}$.}
\end{proof}
...
\begin{lemma}[reflection with the twist]
  ...
  \lean{QuadraticIterates.not_isSquare_betaInt_of_dvd_reflNum,
    QuadraticIterates.not_isSquare_betaInt_of_dvd_reflNum_of_two_le,
    QuadraticIterates.not_isSquare_betaInt_of_squarefree_of_dvd_reflNum_one,
...
\begin{proof}
  ...
  \formalnote{\lname{not\_isSquare\_betaInt\_of\_dvd\_reflNum}: hypotheses $n \ge 2$,
  $n' = \rad(n)$, $n = k n'$, $M \mid N_k$ (\lname{reflNum r s ε k}, the numerator
  $w_k s^{2^{k-1}} + w_{k+1}$, coprime to $s$ and positive when $w$ is; so $\bar s$ is a unit of
  $\ZZ/M\ZZ$, whose inverse the proof obtains from
  \lname{ZMod.isUnit\_intCast\_of\_isCoprime\_of\_dvd}), and $-\bar s^{F}$ not a
  ...
  forms: \lname{not\_isSquare\_betaInt\_of\_dvd\_reflNum\_of\_two\_le} ($k \ge 2$, $F$ even, so
  $-1$ not a square suffices) and \lname{not\_isSquare\_betaInt\_of\_squarefree\_of\_dvd\_reflNum\_one}
...
\begin{proposition}[the classes (C${}^\pm$)]
  ...
  \uses{def:wseq, def:betaint, lem:rat-reflection}
  \lean{QuadraticIterates.TwoAdicClass, QuadraticIterates.reflNum_emod_of_odd,
    ...
    QuadraticIterates.intCast_wSeq_zmod_eight_of_even, ZMod.intCast_sq_eight_eq_one_of_odd,
    ZMod.intCast_pow_eight_eq_zero_of_even,
    ZMod.intCast_pow_two_pow_sub_one_eight_of_odd,
    Int.not_isSquare_neg_one_zmod_natAbs_of_emod_eight_eq_six,
    Int.not_isSquare_neg_one_zmod_natAbs_of_emod_four_eq_three}
  \leanok
...
\begin{proof}
  ...
  \pmod 4$. The listed classes are exactly those where the residue is as claimed (the
  proposition on the $2$-adic classes of the working notes).
  ...
  ($-1$ is not a square modulo $N_k$, via
  \lname{Int.not\_isSquare\_neg\_one\_zmod\_natAbs\_of\_emod\_eight\_eq\_six} and
  \lname{\ldots\_of\_emod\_four\_eq\_three})
  and the corollary \lname{not\_isSquare\_betaInt\_of\_twoAdicClass\_of\_two\_le}: in the classes, $\beta_n$ is not
  a square for $n \ge 2$ with $n / \rad(n) \ge 2$, by the reflection lemma with the modulus $N_k$
  itself; both are stated with the hypothesis $N_k \ge 0$ in place of a sign world, which the
  theorems supply through \lname{reflNum\_pos}.}
\end{proof}
...
\end{lemma}
\begin{proof}
  \uses{lem:betaint, lem:moebiussign, lem:zmodsquare, lem:rat-reflection}
  \leanok
  ...
  \lname{Squarefree.odd\_sum\_two\_pow\_sub\_one}.}
\end{proof}

\begin{lemma}[reflection at $n_0 = 1$]
  \label{lem:rat-refl1}
  \uses{lem:rat-reflection}
  \lean{QuadraticIterates.not_isSquare_betaInt_of_squarefree_of_dvd_reflNum_one}
  \leanok
...
\begin{lemma}[the modulus $8$ for even $s$]
  \label{lem:rat-mod8}
  \uses{def:wseq, def:betaint}
  \lean{QuadraticIterates.not_isSquare_betaInt_of_squarefree_of_odd_of_even,
    QuadraticIterates.not_isSquare_betaInt_of_squarefree_of_even_of_even,
    ZMod.not_isSquare_intCast_eight_of_odd_of_emod_ne_one}
  \leanok
...
\end{lemma}
\begin{proof}
  \uses{lem:betaint, lem:moebiussign, lem:rat-reflection, prop:rat-twoadic, lem:zmodsquare}
  \leanok
  ...
  makes the integer $PQ$ a square) and \lname{Squarefree.card\_divisors\_eq\_two\_mul}; the
  ...
  \lname{ZMod.not\_isSquare\_intCast\_eight\_of\_odd\_of\_emod\_ne\_one} (an odd integer
...
\begin{theorem}[Theorem R${}^+$]
  \label{thm:rat-pos}
  \uses{def:galois, def:wreath, def:wseq, prop:rat-twoadic}
  \lean{QuadraticIterates.nonempty_mulEquiv_of_neg_lt_of_twoAdicClass_of_squarefreeCertificate,
    QuadraticIterates.nonempty_mulEquiv_of_twoAdicClass_of_squarefreeCertificate,
    QuadraticIterates.SquarefreeCertificate}
  \leanok
...
\begin{proof}
  ...
  \formalnote{The uniform theorem
  \lname{nonempty\_mulEquiv\_of\_twoAdicClass\_of\_squarefreeCertificate} takes the sign
  $\varepsilon$, the positivity of $w$, the class \lname{TwoAdicClass r s ε} of
  Proposition~\ref{prop:rat-twoadic}, ``$-\varepsilon r s$ is not a square'' and a certificate
  \lname{SquarefreeCertificate r s ε}, an inductive predicate with one constructor per mechanism of the
  squarefree levels (\lname{dvd}, \lname{reflNum}, \lname{even}, \lname{gammaTwo}); each takes a
  general modulus $M$ dividing the respective quantity (hence coprime to $s$) and ``not a square
  in \lname{ZMod M}'' in place of the odd prime $p$ with Legendre symbol $-1$ and of the modulus
  $4$, which it subsumes
  (\lname{not\_isSquare\_betaInt\_of\_squarefree\_of\_squarefreeCertificate}). Theorem R${}^+$ is the
  instance $\varepsilon = 1$, $-s < r$, with the positivity of $w$ from
  \lname{wSeq\_pos\_of\_pos} ($r > 0$) and \lname{wSeq\_pos\_of\_neg\_lt} ($-s < r < 0$).
...
\begin{theorem}[Theorem R${}^-$]
  \label{thm:rat-neg}
  \uses{def:galois, def:wreath, def:wseq, prop:rat-twoadic, thm:rat-pos}
  \lean{QuadraticIterates.nonempty_mulEquiv_of_two_mul_le_neg_of_twoAdicClass_of_squarefreeCertificate}
  \leanok
  ...
  $4 \mid R - s$ and $s \equiv 1 \pmod 4$; (SQ${}^-$5) $s$ is even and
  $R - s \not\equiv 1 \pmod 8$ holds. Then $\Om_n \cong \Ct{n}$ for all $n \ge 1$.
\end{theorem}
\begin{proof}
  \uses{lem:rat-shared, lem:rat-reflection, prop:rat-twoadic, lem:rat-ps, lem:rat-refl1, lem:rat-mod8,
    lem:rat-g2}
  ...
  |\beta_n|$), clause (SQ${}^-$4) by Lemma~\ref{lem:rat-g2} and clause (SQ${}^-$5) by
  Lemma~\ref{lem:rat-mod8} in Stoll's world.
  \formalnote{The instance $\varepsilon = -1$ at $(-r, s)$ of the uniform theorem of
  Theorem~\ref{thm:rat-pos}, with $2s \le -r$, the class \lname{TwoAdicClass (-r) s (-1)}, the
  certificate \lname{SquarefreeCertificate (-r) s (-1)} and the positivity of $w^S$ from
  \lname{wSeq\_pos\_of\_two\_mul\_le}; the two worlds share every statement through the general
  \lname{gammaSeq} lemmas over \lname{ZMod M}, specialized only at the end. Clause (SQ${}^-$4) is
  Lemma~\ref{lem:rat-g2} with $\varepsilon = -1$ for $n \ge 3$, and $\beta^S_2 = w^S_2 = |r| - s$
  is not a square by hypothesis. Clause (SQ${}^-$5) is the constructor \lname{even} of the
  certificate, Lemma~\ref{lem:rat-mod8} in Stoll's world; the working notes state the lemma for
  both worlds but list the clause only in Theorem R${}^+$.}
\end{proof}
...
\end{lemma}
\begin{proof}
  \uses{lem:li-modsq, lem:betaint, lem:gamma-away, lem:moebiussign, lem:rat-reflection,
    lem:li-parity, lem:zmodsquare}
  \leanok
...
The order below is the order in which the material was formalized, one verified commit per
item.
\begin{enumerate}
...
\item \lname{Rational/Sequence.lean}: Definition~\ref{def:wseq} with the identity over a general
  ...
  Definition~\ref{def:betaint}, Lemma~\ref{lem:betaint}, the sign worlds.
...
\item \lname{Rational/GammaTwo.lean}: Lemma~\ref{lem:rat-g2}, independent of the Galois side.
\item The Galois side for $a \in \QQ$ (Theorem~\ref{thm:rat-section1}), in the four files
  \lname{Iterates.lean} (polymorphic \lname{cSeq}, \lname{bSeq}; rational
  \lname{splittingField}, \lname{GaloisGroup}; cast lemmas), \lname{Irreducibility.lean}
  (Lemma~1.2 general, growth integral), \lname{DegreeCriterion.lean} (hypothesis swap) and
  \lname{Main.lean} (Section~1 over $\QQ$, integer corollaries) of Chapter~1, together with the
  comparator challenges, after the author's approval of the change of \lname{GaloisGroup}.
...
\item \lname{Rational/Main.lean}: the certificate predicate \lname{SquarefreeCertificate}, the uniform
  theorem \lname{nonempty\_mulEquiv\_of\_twoAdicClass\_of\_squarefreeCertificate}, and Theorems
  \ref{thm:rat-pos} and \ref{thm:rat-neg} as its two sign instances.
\item The audit of the chapter's code, in four verified rounds of one idea each: certificates
  derived from the divisibility data instead of assumed (the unit $s u = 1$ modulo $M$); the
  general facts extracted from the proofs into the mirror files (exponent identities, the sign
  partition, squares times powers of units, coprimality and positivity of the Möbius products);
  placement and naming (\lname{SquarefreeCertificate}, the reflection lemma named after
  \lname{reflNum}, the residue lemmas modulo $8$ in \lname{Mathlib/Data/ZMod.lean} in the
  \lname{Odd}/\lname{Even} currency); style and documentation.
\end{enumerate}
