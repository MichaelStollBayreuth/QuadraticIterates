...
\begin{lemma}
  ...
  \lean{QuadraticIterates.not_isSquare_betaSeq, QuadraticIterates.gammaSeq_mul_eq_two_mul,
    QuadraticIterates.prod_gammaSeq_mul_eq_neg_prod,
...
\begin{proof}
  ...
  \lname{prod\_gammaSeq\_mul\_eq\_neg\_prod}. The formal proof goes through the single-index
  criterion \lname{not\_isSquare\_betaSeq\_of\_dvd\_add\_two\_mul} of
  Chapter~\ref{chap:li}.}
\end{proof}
...
\begin{proof}
  ...
  $\ZZ/m\ZZ$; this reduction to Lemma~\ref{lem:2.1} is
  \lname{not\_isSquare\_betaSeq\_of\_pos\_of\_not\_isSquare\_neg\_one}, and the cases a)
  and b) are separate statements. The two congruence computations are done in $\ZZ/4\ZZ$ resp.\ $\ZZ/8\ZZ$, over an
